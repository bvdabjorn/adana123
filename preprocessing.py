...
cpu_GHZ = list()
for i in range(0, length_data_frame):
    if 'GHZ' in data_train['cpu_details'][i].split(' '):
        GHZ = data_train['cpu_details'][i].split(' ')[data_train['cpu_details'][i].split(' ').index('GHZ')-1]
        cpu_GHZ.append(GHZ)
    else:
        cpu_GHZ.append(np.nan)

data_train['cpu_GHZ'] = cpu_GHZ

# Linking cpu benchmark results to dataframe
data_train['cpu_benchmark'] = np.NaN
for i in range(len(data_train['cpu_details_2'])):
    for j in range(len(cpu_data['cpu name'])):
        if str(data_train['cpu_details_2'][i]) in str(cpu_data['cpu name'][j]):
            data_train['cpu_benchmark'][i] = cpu_data['passmark cpu score'][j]

 # Linking gpu benchmark results to dataframe
data_train['gpu_benchmark'] = np.nan
for i in range(len(data_train['gpu_type'])):
    for j in range(len(gpu_data['gpu name'])):
        if str(data_train['gpu_type'][i]) in str(gpu_data['gpu name'][j]):
